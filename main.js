...
function initBackgroundMusic() {
    ...
    function playTrack() {
        audio.play()
            .then(() => {
                avatarBtn.innerHTML = '<img src="images/profile.JPG" alt="avatar"><span class="center-icon">⏸</span>';
                avatarBtn.setAttribute('aria-pressed', 'true');
                avatarBtn.setAttribute('aria-label', 'Pause');
            ...
            .catch(error => {
                ...
                avatarBtn.setAttribute('aria-label', 'Play');
    ...
    function getPointerPercentOnRing(evt) {
        const ring = document.querySelector('.progress-ring');
        // Get pointer position relative to center of the SVG
        const rect = ring.getBoundingClientRect();
        const cx = rect.left + rect.width / 2;
        const cy = rect.top + rect.height / 2;
        const x = (evt.touches?.[0]?.clientX ?? evt.clientX) - cx;
        const y = (evt.touches?.[0]?.clientY ?? evt.clientY) - cy;

        // Angle from -PI..PI (0 at right). Convert so 0 is at top, clockwise positive.
        let angle = Math.atan2(y, x); // radians
        angle = angle + Math.PI / 2;  // rotate so 0 at top
        if (angle < 0) angle += Math.PI * 2;
        const percent = angle / (Math.PI * 2); // 0..1 clockwise

        // Optionally ignore taps far outside the ring
        const dist = Math.hypot(x, y);
        const outer = rect.width / 2;
        const inner = outer * 0.6; // Skip if click is too close to center
        if (dist < inner || dist > outer + 20) {
            return null;
        }
        return percent;
    }
    ...
    function pointerDown(evt) {
        if (!evt.target.closest('.circular-player')) return;
        seeking = true;
        const p = getPointerPercentOnRing(evt);
        if (p != null) {
            seekToPercent(p);
        } else {
            // Inside avatar center: treat as simple toggle
            togglePlayPause();
        }
    }
    ...
    // Event listeners
    avatarBtn.addEventListener('click', togglePlayPause);
